...
const DropdownCard = ({title, options, selectedValue, onSelect}: DropdownCardProps) => {
    ...
    return (
        <View className="flex-col px-[3%] mb-[3%]">
            <View
                className="flex-auto justify-center rounded-xl border border-[#D9D9D9] bg-background-dark p-[3%] min-h-[100px]"
            >
                ...
                <View className="flex-row mt-3">
                    {options.map((option, index) => (
                        <TouchableOpacity
                            ...
                            ${selectedValue === option ? 'bg-primary border-primary' : 'bg-white border-[#D9D9D9]'}`}
                        >
